            ...
            return (products.map((product,i) => {
                return(<Col key={i} lg={3} md={3} sm={6} xs={12}>
                    <Thumbnail src={require("../styles/img.jpg")} alt={product.name}>
                        ...
                        <h2 className='priсe'><strong>Priсe:</strong> {product.price} <i className="fa fa-rub" aria-hidden="true"> </i></h2>
                        <p className='button'>
                            <Button id={i} bsStyle="primary" onClick={this.learnMore}>Learn More</Button>&nbsp;
                            <Button bsStyle="primary" onClick={createNotification({type:"success", title:product.name, message:"Added to cart"})}>Add to Basket</Button>&nbsp;
                        </p>
                    </Thumbnail>
                </Col>)})
           )
